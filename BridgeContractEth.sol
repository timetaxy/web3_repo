...
contract BridgeToken is Ownable, Pausable, ReentrancyGuard {
    ...
    function submitBurningTX(IERC20 token, address recipient, uint amount, string calldata txHash) external onlySubmitter transactionExists(txHash) returns (uint256 transactionId) {
    ...
    function signBuringTX(string calldata txHash, bytes calldata signature) external onlyOwner {
